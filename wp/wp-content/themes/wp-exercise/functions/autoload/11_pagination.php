<?php
function get_pagination_array(int $range, bool $add_first_and_last = false)
{
...
function get_pagination_numbers(int $pages, int $paged, int $range)
{
    ...
    for ($i = max(1, $paged - $range); $i <= min($pages, $paged + $range); $i++) $rtn[] = $i;
    if (count($rtn) >= $min_number) return $rtn;

    if ($rtn[0] === 1) {
        for ($i = count($rtn) + 1; $i <= min($pages, $min_number); $i++) $rtn[] = $i;
    } else {
        for ($i = $rtn[0] - 1; $i >= max(1, $pages - $min_number + 1); $i--) $rtn[] = $i;
        sort($rtn);
    }
    return $rtn;
}